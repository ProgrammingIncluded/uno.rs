...
impl Game {
    fn generate(&self) -> Vec<Move> {
        ...
        let mut moves = vec![];
        for i in 0..hand.len() {
            if let Some(m) = Game::playable(top, &hand[i], i, self.turn) {
    ...
    fn simulate(&self, picked: &Move) -> Game {
        ...
        if picked.variant != Variant::DrawDeck {
    ...
    fn playable(
        ...
        // Check if chainable.
        if top_of_deck.variant != playing.variant
